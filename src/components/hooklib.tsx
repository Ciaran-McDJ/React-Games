...
function assert(cond: any, mes: string) {
    //: asserts cond {
    if (!cond) {
        throw new Error(mes);
    }
...
export type GenEffect = Iterator<
    IArguments | any[] | undefined | void // , void | (() => void)
>;
...
export function useGenEffect(effect: GenEffect) {
    ...
    assert(
        ...
        "gen effect must yield arguments or a list of dependencies."
    );
...
export function makeOwnHook<
    F extends (...args: any[]) => GenEffect,
...
export const effectEventListener = effectFactory(
    window.addEventListener as <K extends keyof WindowEventMap>(
        type: K,
        listener: (this: Window, ev: WindowEventMap[K]) => any,
        options?: boolean | AddEventListenerOptions
    ) => void,
    (r, ...args) => window.removeEventListener(...args)
);
...
export abstract class HookedComponent<Props = React.PropsWithChildren<{}>> {
    ...
    >(
        ...
        const Comp = React.forwardRef<Inst, P>((props, ref) => {
            ...
            inst._request_update = React.useReducer(
                (s: {}, arg: REQUEST_UPDATE) => {
                    return {};
                },
                {}
            )[1];
    ...
    private _render_affecting_internal_store = (() => {
        ...
        for (const field of Object.keys(inits || {}) as (keyof this)[]) {
            ...
            store[field] = inits![field] && inits![field]!();
        }
        return store;
    })();
    /**
     ...
     */
    protected static RenderAffecting<
        T extends HookedComponent<any>,
        K extends keyof T
    >(proto: T, field: K, d?: { initializer?(): T[K] }) {
        assert(
...
export function statefulHookedComponent<
    State,
    Reducers extends {
        [K in keyof State]?: (prev: State[K], arg: any) => State[K];
    }
>(initialState: State, reducers?: Reducers) {
    const fields = Object.keys(initialState) as (keyof State)[];
    type Arg<K extends keyof State> = Reducers[K] extends (
        prev: State[K],
        arg: infer A
    ) => State[K]
        ? A
    ...
    abstract class Stateful<P> extends HookedComponent<P> {
        ...
        protected updateState<K extends keyof State>(k: K, arg: Arg<K>) {
            ...
            assert(
                dispatch !== undefined,
                "updateState called before first render."
            );
            dispatch!(arg);
        }
        protected [InternaluseRender](props: P) {
            ...
            for (const field of fields) {
                const reducer = (reducers && reducers[field]) || defaultReducer;
    ...
    return (Stateful as unknown) as {
        new <P>(NEVER_PASSED?: HookedComponent.INIT_ARG<P>): Stateful<P> &
            Readonly<State>;
    };
}
